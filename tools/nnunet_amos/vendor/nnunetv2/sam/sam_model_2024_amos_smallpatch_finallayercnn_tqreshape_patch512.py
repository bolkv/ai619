...
class SAMAdapter_2024_AMOS_SmallPatch_FinalLayerCNN_TQReshape_Patch512(nn.Module):
    ...
    def __init__(
        ...
        checkpoint = os.environ.get("SAM_CHECKPOINT", os.path.join(os.path.dirname(os.path.abspath(__file__)), "sam_checkpoints", "sam_vit_h_4b8939.pth")),
        frames: int=8,
        num_queries: int=24
    ...
    def _get_image_pe(self, image_emb):
        B, C, H, W = image_emb.shape
        pe = self.sam_model.prompt_encoder.pe_layer((H, W)).unsqueeze(0).to(image_emb.device)
        num_frames = self.sam_model.prompt_encoder.num_frames
        pe = pe.repeat(num_frames, 1, 1, 1)
        pe = torch.cat([pe] * (B // num_frames), dim=0)
        return pe
    ...
    def forward(self, input_x, second_stage=False):  #, input_mask):
        ...
        # Store binary prompt mask (exactly what goes into prompt encoder)
        if os.environ.get('SAVE_PROMPT_MASK', '0') == '1':
            self._last_prompt_mask = input_mask.detach()  # (B*T, NQ, H, W), binary 0/1
        ...
        # Store normalized bbox for visualization (before pixel conversion)
        if os.environ.get('SAVE_BBOX', '0') == '1':
            self._last_bbox = bbox_outputs.detach()  # (B*T, NQ, 2, 2) normalized x1y1x2y2
        ...
        low_res_masks, _, output_classes = self.sam_model.mask_decoder(
            ...
            image_pe=self._get_image_pe(image_embeddings[-1]),
        ...
        # Store attention map from final cross-attention if requested
        if os.environ.get('SAVE_ATTN_MAP', '0') == '1':
            transformer = self.sam_model.mask_decoder.transformer
            final_attn = transformer.final_attn_token_to_image
            if hasattr(final_attn, '_attn_map'):
                self._last_attn_map = final_attn._attn_map  # (B*T*NQ, N_heads, N_tokens, 64*64)
        ...
        mask_predictions = rearrange(mask_predictions, 'b nq t h w -> (b t) nq h w') #t num of slice
