...
from nnunetv2.training.loss.matcher import HungarianMatcherAndAux_2_4# HungarianMatcher, HungarianMatcherAndAux, HungarianMatcherAndAux_2_4, HungarianMatcherAndAux_RemoveSamplePoints
...
class MaskSAM_AMOS(nnUNetTrainer):
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            # DiceCE loss for anatomix direct class prediction (like original anatomix training)
            self.anatomix_loss = DC_and_CE_loss(
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        seg_target = batch['target'][0].to(self.device, non_blocking=True)  # (B, 1, T, H, W) class indices
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            # DiceCE loss for anatomix class prediction (direct supervision, no Hungarian matching)
            if 'pred_anatomix_cls' in outputs:
                anatomix_cls = outputs['pred_anatomix_cls']  # (B, num_cls, T, H, W)
                # seg_target: (B, 1, T, H, W) with class indices
                anatomix_dicece_loss = self.anatomix_loss(anatomix_cls, seg_target)
                l = l + anatomix_dicece_loss
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            out = self.network(data, second_stage=False)
    ...
    def run_training(self):
        ...
        pbar = tqdm(range(self.current_epoch, self.num_epochs), desc="Epochs", disable=not is_main)
        for epoch in pbar:
            self.on_epoch_start()

            self.on_train_epoch_start()
            train_outputs = []
            train_pbar = tqdm(range(self.num_iterations_per_epoch), desc=f"Train [Epoch {epoch}]", leave=False, disable=not is_main)
            for batch_id in train_pbar:
                result = self.train_step(next(self.dataloader_train))
                train_outputs.append(result)
                if is_main:
                    train_pbar.set_postfix(loss=f"{float(result['loss']):.4f}")
            self.on_train_epoch_end(train_outputs)

            with torch.no_grad():
                self.on_validation_epoch_start()
                val_outputs = []
                val_pbar = tqdm(range(self.num_val_iterations_per_epoch), desc=f"Val   [Epoch {epoch}]", leave=False, disable=not is_main)
                for batch_id in val_pbar:
                    result = self.validation_step(next(self.dataloader_val))
                    val_outputs.append(result)
                    if is_main:
                        val_pbar.set_postfix(loss=f"{float(result['loss']):.4f}")
                self.on_validation_epoch_end(val_outputs)

            self.on_epoch_end()
            pbar.set_postfix({
                'tr_loss': f"{self.logger.my_fantastic_logging['train_losses'][-1]:.4f}",
                'val_loss': f"{self.logger.my_fantastic_logging['val_losses'][-1]:.4f}",
                'dice': f"{self.logger.my_fantastic_logging['mean_fg_dice'][-1]:.4f}",
            })
