...
- `pancreas_tumor_seg` (MONAI DiNTS, pancreas_ct bundle): Produces a 3-class mask (background / pancreas / tumor) from a 3D abdominal CT, addressing cases such as pancreatic cancer research where the tumor-organ boundary is critical.
...
- Within the same ChatClinic interface, learners can see how the 3D CT modality and the DiNTS architecture are applied to a specific target. `@seg_pancreas` demonstrates a 3-class pancreas+tumor output mask structure.
